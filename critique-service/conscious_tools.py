...
# ---------------------------------------------------------------------------
# Capability tools (bash, file read/write, glob, grep)
# ---------------------------------------------------------------------------
...
def _bash(agent_session: Any, args: dict) -> dict:
    ...
    if not command:
        return {"error": "command is required", "stdout": "", "stderr": "", "exit_code": -1}

    for pat in _DANGEROUS_PATTERNS:
        if pat in command:
            return {"error": f"blocked dangerous pattern: {pat}", "stdout": "", "stderr": "", "exit_code": -1}

    workdir = str(args.get("workdir", "")).strip() or None
    timeout = int(args.get("timeout_s", 30) or 30)
...
def _file_read(agent_session: Any, args: dict) -> dict:
    """Read a file from the filesystem."""
    from pathlib import Path
    path = str(args.get("path", "")).strip()
    if not path:
        return {"error": "path is required", "content": ""}
...
def _file_write(agent_session: Any, args: dict) -> dict:
...
def _glob_tool(agent_session: Any, args: dict) -> dict:
    """Find files matching a glob pattern."""
    from pathlib import Path
    pattern = str(args.get("pattern", "")).strip()
    if not pattern:
        return {"error": "pattern is required", "files": []}
    base = str(args.get("base", "")).strip() or "."
    limit = int(args.get("limit", 100) or 100)
    try:
        matched = []
        for p in Path(base).rglob(pattern):
            if p.is_file() and len(matched) < limit:
                matched.append(str(p.relative_to(base) if base != "." else p))
        return {"files": matched, "count": len(matched), "truncated": len(matched) >= limit}
    except Exception as exc:
        return {"error": str(exc), "files": []}


def _grep(agent_session: Any, args: dict) -> dict:
    ...
    try:
        ...
        if base.is_file():
            ...
            files_to_check = list(base.rglob(include))
            ...
            try:
                for i, line in enumerate(fp.read_text(encoding="utf-8").splitlines(), 1):
                    if _re.search(pattern, line):
                        rel = str(fp.relative_to(base)) if base != Path(".") else str(fp)
...
CONSCIOUS_TOOLS: list[dict] = [
    ...
    # ------------------------------------------------------------------
    # Capability tools (available to all agents)
    # ------------------------------------------------------------------
    {
        "name": "bash",
        "description": (
            "Execute a shell command on the server. Returns stdout, stderr, and exit code. "
            "Use this for running commands, compiling code, managing files, git operations, "
            "and any other shell task. Dangerous patterns (rm -rf /, shutdown, etc.) are blocked."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "file_read",
        "description": "Read the contents of a file. Returns the file content and size.",
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "absolute or relative file path (required)"},
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "pattern": {"type": "string", "description": "glob pattern to match (required)"},
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "include": {"type": "string", "default": "*",
                            "description": "glob for files to include when searching a directory"},
