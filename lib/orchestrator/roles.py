    ...
    *,
    ...
    if role is Role.GENERATOR:
        return _merge_generator(stage_name, context, raw_output, shard_index,
                                nonce=nonce, output_format=output_format)
...
def _merge_generator(
    ...
    *, nonce: str = "", output_format: str | None = None,
) -> dict:
    ...
    When output_format == "files", the raw output is parsed into multi-file
    artifacts (marker blocks + nonce). The artifacts are stored on
    context["artifacts"] and the body is set to a path-labelled concatenation
    so the text-based judge rules still apply.
    """
    text = raw.strip()
    if not text:
    ...
    if output_format == "files":
        ...
        files = _artifacts.parse_artifacts(text, nonce)
        text = _artifacts.files_to_body(files)
